...
def min(journalists)
    a = journalists.sort_by(&:length) 
    puts a.first
end

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère) //pas fini
def five(journalists)
    journalists.each_index {|i| print i, "--"}
end
...
def sorting(journalists)
    puts journalists.each_index {|i| print}
end
